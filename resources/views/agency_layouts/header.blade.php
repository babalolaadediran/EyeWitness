...
<header class="header">
	<nav class="navbar navbar-expand-lg px-4 py-2 bg-white shadow">
		...
		<a href="{{ url('agency/home') }}" class="navbar-brand font-weight-bold text-uppercase text-base">Mopani Hotspot Reporter</a>
			...
			<li class="nav-item dropdown ml-auto">
				...
				<div aria-labelledby="userInfo" class="dropdown-menu">
					<a href="javascript:void(0);" class="dropdown-item">
						<strong class="d-block text-uppercase headings-font-family">{{ $agency->name }}</strong>
						<small>{{ $agency->email }}</small>
					</a>
					<div class="dropdown-divider"></div>
					<a href="{{ url('agency/profile') }}" class="dropdown-item">Profile</a>
					<div class="dropdown-divider"></div>
					<a href="javascript:void(0);" id="logout-link" class="dropdown-item">Logout</a>
					<form method="POST" id="logout-form" action="{{ url('agency/logout') }}">
						@csrf
					</form>
				</div>
			</li>
